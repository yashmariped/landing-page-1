...
(() => {
  ...
  const allowedExternal = Object.freeze({linkedin:'https://www.linkedin.com/in/yashmaripeddi/',github:'https://github.com/yashmariped'});
  const friendlyFiles = Object.freeze({about:'about.txt',whoami:'about.txt',experience:'experience.txt',skills:'skills.txt',projects:'projects.txt',education:'education.txt',recommendations:'recommendations.txt',learning:'learning.txt',contact:'contact.txt'});

  function print(text, className = '') { const p=document.createElement('p'); if(className)p.className=className;p.textContent=String(text);output.appendChild(p);output.scrollTop=output.scrollHeight; }
  function setPrompt(){promptEl.textContent=`yash@yashhacks:${cwd}$`;}
  function openTrusted(url){const parsed=new URL(url);if(parsed.protocol!=='https:')return;window.open(parsed.href,'_blank','noopener,noreferrer');}

  function run(rawInput){
    const normalized=rawInput.slice(0,MAX_COMMAND_LENGTH).trim();const parts=normalized.split(/\s+/u);const cmd=(parts.shift()||'').toLowerCase();const arg=(parts[0]||'').toLowerCase();if(!cmd)return;
    ...
    if(cmd==='cd'){const dest=arg.replace(/\/+$/u,'');if(!dest||dest==='~'||dest==='..')cwd='~';else if(dest==='certs')cwd='certs';else{print(`cd: no such directory: ${dest}`,'muted');return;}setPrompt();return;}
    if(cmd==='cat'){if(cwd==='~'&&arg==='certs'){print(certSummary);return;}const source=cwd==='certs'?certFiles:files;const resolved=cwd==='~'&&Object.prototype.hasOwnProperty.call(friendlyFiles,arg)?friendlyFiles[arg]:arg;if(Object.prototype.hasOwnProperty.call(source,resolved))print(source[resolved]);else print(`cat: ${arg||'(missing file)'}: No such file`,'muted');return;}
  ...
  form.addEventListener('submit',(event)=>{event.preventDefault();const raw=input.value.slice(0,MAX_COMMAND_LENGTH).trim();if(!raw)return;print(`${promptEl.textContent} ${raw}`,'prompt');history.push(raw);if(history.length>MAX_HISTORY)history.shift();historyIndex=history.length;input.value='';run(raw);});
  input.addEventListener('keydown',(event)=>{if(event.key==='ArrowUp'){event.preventDefault();if(historyIndex>0)input.value=history[--historyIndex]||'';}else if(event.key==='ArrowDown'){event.preventDefault();if(historyIndex<history.length)historyIndex+=1;input.value=history[historyIndex]||'';}else if(event.key==='Tab'){event.preventDefault();const value=input.value.trim();if(value.startsWith('cat ')){const query=value.slice(4);const names=cwd==='certs'?Object.keys(certFiles):[...Object.keys(files),'certs'];const match=names.find((name)=>name.startsWith(query));if(match)input.value=`cat ${match}`;}else{const match=commands.find((command)=>command.startsWith(value));if(match)input.value=match;}}});
  document.getElementById('terminal')?.addEventListener('click',()=>input.focus());setPrompt();
})();
